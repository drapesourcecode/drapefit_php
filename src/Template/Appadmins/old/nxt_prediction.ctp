...
//            }
                                            if (!empty($p_c_list->transactions_id)) {
                                                ...
                                                if ($p_c_list->parent_fix->try_new_items_with_scheduled_fixes == 1) {
                                                    if (($p_c_list->parent_fix->how_often_would_you_lik_fixes == 1) && date('Y-m-d', strtotime('first day of +' . ($p_c_list->parent_fix->how_often_would_you_lik_fixes + 1) . ' month', strtotime($p_c_list->created_dt))) <= $next_month) {
                                                        ?>
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <a href="<?= HTTP_ROOT; ?>appadmins/prediction_matching/<?= $p_c_list->id; ?>" data-placement="top" class="btn btn-info"><i class="fa fa-magic" aria-hidden="true"></i> Matching</a> 
                                                                <br>
...
//                                                    echo "<br>" . $p_c_list->user_id . " - " . $p_c_list->id . ' - ' . $p_c_list->parent_fix->how_often_would_you_lik_fixes . ' - ' . date('Y-m-d', strtotime($p_c_list->created_dt)) . ' - ' . date('Y-m-d', strtotime('first day of +' . $p_c_list->parent_fix->how_often_would_you_lik_fixes . ' month', strtotime($p_c_list->created_dt))) . ' - ' . $p_c_list->parent_detail->first_name . ' ' . $p_c_list->parent_detail->last_name;
                                                    } else if (($p_c_list->parent_fix->how_often_would_you_lik_fixes == 2) && (date('Y-m-d', strtotime('first day of +' . ($p_c_list->parent_fix->how_often_would_you_lik_fixes + 2) . ' month', strtotime($p_c_list->created_dt))) == $next_month) || (date('Y-m-d', strtotime('first day of +' . ($p_c_list->parent_fix->how_often_would_you_lik_fixes + 2) . ' month', strtotime($p_c_list->created_dt))) == $prev_month)) {
                                                        ?>
                                                        <tr>
                                                        ...
                                                        <?php
//                                                    echo "<br>" . $p_c_list->user_id . " - " . $p_c_list->id . ' - ' . $p_c_list->parent_fix->how_often_would_you_lik_fixes . ' - ' . date('Y-m-d', strtotime($p_c_list->created_dt)) . ' - ' . date('Y-m-d', strtotime('first day of +' . $p_c_list->parent_fix->how_often_would_you_lik_fixes . ' month', strtotime($p_c_list->created_dt))) . ' - ' . $p_c_list->parent_detail->first_name . ' ' . $p_c_list->parent_detail->last_name;
                                                    } else if (($p_c_list->parent_fix->how_often_would_you_lik_fixes == 3) && date('Y-m-d', strtotime('first day of +' . ($p_c_list->parent_fix->how_often_would_you_lik_fixes + 3) . ' month', strtotime($p_c_list->created_dt))) == $next_month) {
                                                        ?>
                                                        <tr>
...
//            echo "</pre>";
                                            if (!empty($p_c_list->transactions_id)) {
                                                if ($p_c_list->kid_fix->try_new_items_with_scheduled_fixes == 1) {
                                                    if (($p_c_list->kid_fix->how_often_would_you_lik_fixes == 1) && date('Y-m-d', strtotime('first day of +' . $p_c_list->kid_fix->how_often_would_you_lik_fixes + 1 . ' month', strtotime($p_c_list->created_dt))) <= $next_month) {
                                                        ?>
                                                        <tr>
                                                            ...
                                                            <td>1</td>
                                                            <td><?= date('Y-' . $one_nxt_month . '-d', strtotime($p_c_list->created_dt)); ?></td>
                                                            <td><a href="<?= HTTP_ROOT; ?>appadmins/prediction_matching/<?= $p_c_list->id; ?>" data-placement="top" class="btn btn-info"><i class="fa fa-magic" aria-hidden="true"></i> Matching</a> </td>
                                                        </tr>
...
//                                                    echo "<br>" . $p_c_list->user_id . " - " . $p_c_list->kid_id . " - " . $p_c_list->id . ' - ' . $p_c_list->kid_fix->how_often_would_you_lik_fixes . ' - ' . date('Y-m-d', strtotime($p_c_list->created_dt)) . ' - ' . date('Y-m-d', strtotime('first day of +' . $p_c_list->kid_fix->how_often_would_you_lik_fixes . ' month', strtotime($p_c_list->created_dt))) . " - " . $p_c_list->kid_detail->kids_first_name;
                                                        ...
                                                        <?php
//                                                    echo "<br>" . $p_c_list->user_id . " - " . $p_c_list->kid_id . " - " . $p_c_list->id . ' - ' . $p_c_list->kid_fix->how_often_would_you_lik_fixes . ' - ' . date('Y-m-d', strtotime($p_c_list->created_dt)) . ' - ' . date('Y-m-d', strtotime('first day of +' . $p_c_list->kid_fix->how_often_would_you_lik_fixes . ' month', strtotime($p_c_list->created_dt))) . " - " . $p_c_list->kid_detail->kids_first_name;
                                                    } else if (($p_c_list->kid_fix->how_often_would_you_lik_fixes == 3) && date('Y-m-d', strtotime('first day of +' . ($p_c_list->kid_fix->how_often_would_you_lik_fixes + 3) . ' month', strtotime($p_c_list->created_dt))) == $next_month) {
                                                        ?>
                                                        <tr>
