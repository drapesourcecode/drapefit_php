...
//        $new_userdetails = $userdetails;
    ...
    public function profile($param = null) {
        ...
        $this->set(compact('rowname', 'settings', 'settingsEmailTempletes', 'row', 'user', 'row', 'options', 'param', 'user_id', 'paymentMode', 'sendle', 'shipping_mode', 'stamps'));
    }
...
//        exit;
    ...
    public function browseProducts($payment_id) {
        ...
        if (!empty($prev_products)) {
            /* $product_list */$product_list1 = $this->InProducts->find('all')->contain(['brand'])->order(['InProducts.id' => 'DESC'])->where(['OR'=>['is_clearance' => 2,'is_clearance IS' => NULL], 'InProducts.available_status !=' => 2,'InProducts.is_active !=' => 0,'InProducts.profile_type' => $gender, 'InProducts.prod_id NOT IN' => $prev_products, 'InProducts.quantity >' => 0, 'InProducts.match_status' => 2])->group('prod_id');
        } else {
            /* $product_list */$product_list1 = $this->InProducts->find('all')->contain(['brand'])->order(['InProducts.id' => 'DESC'])->where(['OR'=>['is_clearance' => 2,'is_clearance IS' => NULL], 'InProducts.available_status !=' => 2,'InProducts.is_active !=' => 0, 'InProducts.profile_type' => $gender, 'InProducts.quantity >' => 0, 'InProducts.match_status' => 2])->group('prod_id');
        }
        if (!empty($_GET['search_for']) && !empty($_GET['search_data'])) {
            if ($_GET['search_for'] == "product_name1") {
                $get_data_value = trim($_GET['search_data']);
...
//                            $this->boxUpdate($planId);
    ...
    public function postComment(){        
        ...
        if ($this->request->is('post')) {
            ...
            $newRw = $this->ProductComments->newEntity();
            $newRw = $this->ProductComments->patchEntity($newRw, $postArr);
            $this->ProductComments->save($newRw);
