...
class UsersController extends AppController {
     ...
     public function adminlogin() {
        ...
        if ($this->request->is('post')) {
            ...
            if ($data['email'] == "") {
                ...
                if ($user) {
                    if ($data['email']) {
                        $isactive_check = $this->SupplyUsers->find('all')->where(['SupplyUsers.email' => $data['email'], 'SupplyUsers.is_active' => true, 'SupplyUsers.type IN' => [1, 3]]);
    ...
    public function registration() {
        ...
        if ($this->request->is('post')) {
            $data = $this->request->data;
            $exitEmail = $this->SupplyUsers->find('all')->where(['email' => $data['email']])->count();
            if($exitEmail >= 1){
    ...
    public function forgot() {
        ...
        if ($this->request->is(['post'])) {
            ...
            if ($data['email'] == "") {
                ...
                $users = $this->SupplyUsers->find('all')->where(['SupplyUsers.email' => $data['email']]);
                $user = $users->first();
                if ($users->count() > 0) {
                    $this->SupplyUsers->query()->update()->set(['qstr' => $user->unique_id])->where(['id' => $user->id])->execute();
...
//                    
    ...
    public function changePassword($uniq = null) {
        ...
        if ($this->request->is('post')) {
            $data = $this->request->data;
            $user = $this->SupplyUsers->newEntity();
            $checkSetPassword = $this->SupplyUsers->find('all')->where(['SupplyUsers.unique_id' => $data['uniq']])->first();
            ...
            if ($data['New_Password'] == "") {
                ...
                $users = $this->SupplyUsers->find('all')->where(['SupplyUsers.unique_id' => $data['uniq']]);
                $uniq = $data['uniq'];
...
//          echo 'hi';
            ...
            $query = $this->SupplyUsers->find('all')
                    ->select(['SupplyUsers.id', 'SupplyUsers.email'])
                    ->where(['SupplyUsers.email' => $email]);
            $count = $query->count();
            if ($count) {
    ...
    public function login() {
        ...
        if ($this->request->is('post')) {
            ...
            if ($data['email'] == "") {
                ...
                if ($user) {
                    if ($data['email']) {
                        $isactive_check = $this->SupplyUsers->find('all')->where(['SupplyUsers.email' => $data['email'], 'SupplyUsers.is_active' => true, 'SupplyUsers.type IN' => [3]]);
