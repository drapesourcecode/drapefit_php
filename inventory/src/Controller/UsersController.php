    ...
    public function adminlogin() {
        ...
        if ($this->request->is('post')) {
            ...
            if ($superadmin_checkcount > 0) {
                $isactive_check = $this->Users->find('all')->where(['email' => $data['email'], 'Users.type IN' => [1, 7]]);
            ...
            if ($data['email'] == "") {
                ...
                if ($user) {
                    if ($data['email']) {
                        $isactive_check = $this->Users->find('all')->where(['email' => $data['email'], 'is_active' => true, 'type IN' => [7, 1]]);
    ...
    public function login() {
        ...
        if ($this->request->is('post')) {
            ...
            if ($data['email'] == "") {
                ...
                if ($user) {
                    if ($data['email']) {
                        $isactive_check = $this->Users->find('all')->where(['email' => $data['email'], 'is_active' => true, 'type IN' => [7]]);
